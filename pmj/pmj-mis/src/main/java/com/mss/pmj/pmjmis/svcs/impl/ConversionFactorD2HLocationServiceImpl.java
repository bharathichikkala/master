...
import com.mss.pmj.pmjmis.svcs.ConversionFactorD2HLocationService;

@Service
@SuppressWarnings("rawtypes")
public class ConversionFactorD2HLocationServiceImpl implements ConversionFactorD2HLocationService {

	private static Logger log = LoggerFactory.getLogger(ConversionFactorD2HLocationServiceImpl.class);
	...
	@Override
	public TargetVsActualResponse<D2HConversionFactorStateData> getLocationD2HStateTargetVsActual(String stateName,
			String clusterName, String startDate, String endDate) {

		log.info("getting Conversion Factors Location wise target vs actual");
		...
		try {
			...
			if (role.getName().equals("ADMIN")) {
				...
				locationList = locationRepo.findByStateAndClusterAndChannel(stateName, clusterName, channel);
			} else {
				...
				if (user.getEmpCode() != null) {
					...
					locationList = locationRepo.locationByStateAndClusterAndChannelAndManager(stateName,clusterName, channel, manager.getId());
			...
			for (Location location1 : locationList) {
				...
				location.add(location1.getLocationCode());

			}

			D2HConversionFactorStateData<ConversionFactorD2HStateDetails> targets = conversionFactorD2HTargets(
					locationList, startDate, endDate);

			D2HConversionFactorStateData<ConversionFactorD2HStateDetails> actuals = conversionFactorD2HActuals(
					locationList, startDate, endDate, channel);

			d2HDetails.put("locations", locationList);
			...
			response.setError(EnumTypeForErrorCodes.SCUS841.name(), EnumTypeForErrorCodes.SCUS841.errorMsg());
			response.setMessage(EnumTypeForErrorCodes.SCUS841.errorMsg());
	...
	public D2HConversionFactorStateData<ConversionFactorD2HStateDetails> conversionFactorD2HTargets(
			List<Location> locationObj, String startDate, String endDate) {

		log.info("getting Conversion Factors D2H Targets Location wise targets");
		...
		try {
			...
			for (Location location : locationObj) {
				...
				if (!d2hTeams.isEmpty()) {
					...
					targetGoldConversion.add(0.0);
					targetDiamondConversion.add(0.0);
			...
			log.error("Failed to get Conversion Factors D2H Targets Location wise targets", e);
		}
		return data;
	}

	public D2HConversionFactorStateData<ConversionFactorD2HStateDetails> conversionFactorD2HActuals(
			List<Location> locationObj, String startDate, String endDate, Channel channel) {

		log.info("get Conversion Factors For Location wise D2H actuals");
		...
		try {
			...
			log.error("Failed to get Conversion Factors For Location wise D2H actuals", e);
		}
		return data;
