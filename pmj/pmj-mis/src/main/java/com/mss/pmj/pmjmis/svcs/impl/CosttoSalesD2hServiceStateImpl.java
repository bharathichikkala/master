...
import com.mss.pmj.pmjmis.svcs.CosttoSalesD2hServiceState;

@RestController
public class CosttoSalesD2hServiceStateImpl implements CosttoSalesD2hServiceState {

	private static Logger log = LoggerFactory.getLogger(CosttoSalesD2hServiceStateImpl.class);
	...
	public CostpricetoSalesResponse<CostPricetoSalesDataD2hState> getcostpricetoSalesd2hbyMonth(String state,
			String startDate, String endDate) {
		log.info("Cost Price to Sales Implementation by Month");

		CostpricetoSalesResponse<CostPricetoSalesDataD2hState> costPricetoSalesDataD2hState = new CostpricetoSalesResponse<>();

		CostPricetoSalesDataD2hState states = new CostPricetoSalesDataD2hState();
		...
		try {
			...
			List<Location> allclusters = new ArrayList<>();
			...
			if (role.getName().equalsIgnoreCase(a)) {
				...
				allclusters = locationRepo.clusterByStateAndChannel(state, channel);
			} else {
				...
				if (user.getEmpCode() != null) {
					...
					allclusters = locationRepo.groupByChannelandManger(channel, manager.getId());
			...
			for (Location location : allclusters) {

				locationsnames.add(location.getCluster());

				double goldTag = 0.0;
				double goldSale = 0.0;
				double diamondTag = 0.0;
				double diamondSale = 0.0;
				List<Sales> salesActualsList = salesRepo.findByStartDateAndEndDateAndCluster(startDate, endDate,
						location.getCluster());
			...
			states.setTotalsaleprice(totalSalePrice);
			states.setTotalcostprice(totalcostPrice);
			Double totalmargin = ((totalSalePrice - totalcostPrice) / totalcostPrice) * 100;
			states.setTotalmargin((totalmargin.isNaN() || totalmargin.isInfinite()) ? 0.0 : totalmargin);
			states.setD2hdetails(costpricetosalesdetailsd2h);
			states.setSaleprice(salePrice1);
			states.setCostprice(costPrice1);
			states.setState(state);

			costPricetoSalesDataD2hState.setStatus(HttpServletResponse.SC_OK);
			costPricetoSalesDataD2hState.setMessage("Cost d2h month successful");
			costPricetoSalesDataD2hState.setFromDate(startDate);
			costPricetoSalesDataD2hState.setToDate(endDate);
			costPricetoSalesDataD2hState.setData(states);
		} catch (Exception e) {
			log.info("cluster1");
		}
		return costPricetoSalesDataD2hState;
	}

	public CostpricetoSalesResponse<CostPricetoSalesDataD2hState> getcostpricetoSalesd2hbyDay(String state,
			String startDate, String endDate) {
		...
		try {
			...
			for (Location locations : allclusters) {
				...
				locationsnames.add(locations.getCluster());
			}
			...
			states.setTotalsaleprice(totalSalePrice1);
			states.setTotalcostprice(totalcostPrice);
			Double totalmargin = ((totalSalePrice1 - totalcostPrice) / totalcostPrice) * 100;
			states.setTotalmargin((totalmargin.isNaN() || totalmargin.isInfinite()) ? 0.0 : totalmargin);
			states.setD2hdetails(costpricetosalesdetailsd2h);
			states.setSaleprice(salePrice1);
			states.setCostprice(costPrice1);
			states.setState(state);
			costPricetoSalesDataD2hState.setStatus(HttpServletResponse.SC_OK);
			costPricetoSalesDataD2hState.setMessage("Cost d2h day successful");
			costPricetoSalesDataD2hState.setFromDate(startDate);
			costPricetoSalesDataD2hState.setToDate(endDate);
			costPricetoSalesDataD2hState.setData(states);
		} catch (Exception e) {
	...
	// By Week
	public CostpricetoSalesResponse<CostPricetoSalesDataD2hState> getcostpricetoSalesd2hbyWeek(String state,
			String startDate, String endDate) {
		log.info("Cost Price to Sales Implementation");
		...
		try {
			...
			costPricetoSalesDataD2hState.setStatus(HttpServletResponse.SC_OK);
			costPricetoSalesDataD2hState.setMessage("Cost d2h week successful");
			costPricetoSalesDataD2hState.setFromDate(startDate);
			costPricetoSalesDataD2hState.setToDate(endDate);
			costPricetoSalesDataD2hState.setData(states);
		} catch (Exception e) {
	...
	@Override
	public CostpricetoSalesResponse<CostPricetoSalesDataD2hState> getcostpricetoSalesd2h(String state, String startDate,
			String endDate) {
		...
		try {
			...
			if (betweenDays < 15) {
				// daywise
				costPricetoSalesDataD2hState = getcostpricetoSalesd2hbyDay(state, startDate, endDate);
			} else if (betweenDays >= 15 && betweenDays <= 60) {
				// week wise
				costPricetoSalesDataD2hState = getcostpricetoSalesd2hbyWeek(state, startDate, endDate);
			} else {
				// month wise
				costPricetoSalesDataD2hState = getcostpricetoSalesd2hbyMonth(state, startDate, endDate);
			}
		} catch (Exception e) {
			costPricetoSalesDataD2hState.setError(EnumTypeForErrorCodes.SCUS110.name(),
					EnumTypeForErrorCodes.SCUS110.errorMsg());
			costPricetoSalesDataD2hState.setMessage("Failed to get costPrice to Sales Margins");
			log.error(utils.toJson(costPricetoSalesDataD2hState.getError()), e);
		}
		return costPricetoSalesDataD2hState;
	}
