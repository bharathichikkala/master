...
import com.mss.pmj.pmjmis.svcs.ConversionFactorD2HClusterService;

@Service
@SuppressWarnings("rawtypes")
public class ConversionFactorD2HClusterServiceImpl implements ConversionFactorD2HClusterService {

	private static Logger log = LoggerFactory.getLogger(ConversionFactorD2HClusterServiceImpl.class);
	...
	public TargetVsActualResponse<D2HConversionFactorStateData> getLocationD2HStateTargetVsActual(String stateName,
		...
		log.info("get Conversion Factors State target vs actual sales groupby between startDate and endDate");

		TargetVsActualResponse<D2HConversionFactorStateData> response = new TargetVsActualResponse<>();

		D2HConversionFactorStateData data = new D2HConversionFactorStateData();
		...
		try {
			...
			D2HConversionFactorStateData targets = conversionFactorD2HTargets(clusters, startDate, endDate, channel,
					user, role);

			D2HConversionFactorStateData actuals = conversionFactorD2HActuals(clusters, startDate, endDate, channel,
					user, role);

			d2HDetails.setClusters(clusterList);

			d2HDetails.setTargets(((ConversionFactorD2HStateDetails) targets.getDetails()).getTargets());

			d2HDetails.setActuals(((ConversionFactorD2HStateDetails) actuals.getDetails()).getActuals());

			d2HDetails.setWalkins(((ConversionFactorD2HStateDetails) actuals.getDetails()).getWalkins());
	...
	public D2HConversionFactorStateData conversionFactorD2HTargets(List<Location> clusterObj, String startDate,
		...
		try {
			...
			for (Location cluster : clusterObj) {
				...
				for (Location location : d2hByState) {
	...
	public D2HConversionFactorStateData conversionFactorD2HActuals(List<Location> clusterObj, String startDate,
			String endDate, Channel channel, User user, Role role) {
