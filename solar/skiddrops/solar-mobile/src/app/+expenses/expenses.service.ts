...
export class ExpensesServices {
    ...
    addExpenses(obj) {
        return this.http.post(endponitConfig.SOLAR_EXPENSES + 'addExpenses', obj, { headers: this.headers })
            .map((response: any) => response);
    }

    getAllExpensesByLoadId(id) {
        return this.http.get(endponitConfig.SOLAR_EXPENSES + 'getDetailsByLoad/' + id, { headers: this.headers })
            .map((response: Response) => response.json()).catch(this.handleError);
    }

    getLoadNumberByDriverid(id) {
        return this.http.get(endponitConfig.LOAD_API + 'acceptedcompletedloads/' + id, { headers: this.headers })
            .map((response: Response) => response.json()).catch(this.handleError);
    }
