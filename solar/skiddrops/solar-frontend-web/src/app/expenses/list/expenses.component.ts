...
export class ExpensesComponent implements OnInit {
    ...
    // constructor(private http: Http, private expensesService: Expenseservice, private router: Router) {
    ...
    ngOnInit() {
        // this.expensesService.getLoads().subscribe((data) => {
        //     this.loadsList = data;
        // })
    }

    getExpensesDetailsbyLoadnum() {
        this.expensesService.getExpensesByloadNumber(this.loadQueryData.loadNumber).subscribe((data) => {
            this.loadsList = data
        })
    }
    ...
    options = {
        ...
        ajax: (data, callback, settings) => {
            this.http.get(endponitConfig.EXPENSES_API + 'getAllExpenses', { headers: this.driverHeaders })
        ...
        rowCallback: (row: Node, data: any | Object, index: number) => {
            ...
            $('a.editor_view', row).bind('click', () => {
                // self.editUser(data);
                self.gotoExpensesDetails(data);
            });
        },
        columns: [
            { data: 'loadNumber.loadNumber', responsivePriority: 1 }, { data: 'loadNumber.driver.firstName', responsivePriority: 5 }, { data: 'noOfExpenses', responsivePriority: 4 },
            { data: 'totalAmount', responsivePriority: 3 }, {
                data: null,
                orderable: false,
    ...
    gotoExpensesDetails(obj) {
        this.router.navigate(['./expenses/details/', obj.loadNumber.loadNumber])
    }
