...
export class FoodAddComponent {
  ...
  constructor(private fb: FormBuilder, private router: Router, private motelService: FoodService) {
  ...
  onchange(eventChange: any, food: any) {
    food.latitude = '';
    food.longitude = '';
    this.motelService.getFoodAddress(food.address).subscribe(response => {
  ...
  submitForm(value) {
    if (this.complexForm.valid) {
      ...
      this.motelService.addMotel(motelObj).then(response => {
        if (response.data != null) {
          this.foodAddSuccess = 'Motel added sucessfully'
          let that = this;
          setTimeout(() => {
            this.foodAddSuccess = ''
