...
export class FoodUpdateComponent {
  ...
  constructor(private cdr: ChangeDetectorRef, private route: ActivatedRoute, private fb: FormBuilder,
    private router: Router,
    private motelSevice: FoodService) {
  ...
  onchange(eventChange: any, food: any) {
    food.latitude = '';
    food.longitude = '';
    this.motelSevice.getFoodAddress(food.address).subscribe(response => {
  ...
  ngOnInit() {
    this.route.params.forEach((params: Params) => {
      if (params['Id'] !== undefined) {
        ...
        this.motelSevice.getMotelById(this.foodId).subscribe(data => {
          this.food = data;
  ...
  submitForm(value) {
    if (this.complexForm.valid) {
      ...
      this.motelSevice.updateFoodStation(foodObject).then(response => {
        if (response.data != null) {
          this.foodAddSuccess = 'Motel updated sucessfully'
          let that = this;
          setTimeout(() => {
            this.foodAddSuccess = ''
