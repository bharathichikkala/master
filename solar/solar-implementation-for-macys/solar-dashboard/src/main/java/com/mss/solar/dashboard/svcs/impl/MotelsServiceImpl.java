		...
		try {
			...
			if (id == 0) {
				...
				for (FoodCourt foodCourt : foodCourtsList) {
					...
					Double distance = locationsService.getDistanceByRadian(latitide, longitude, courtLatitide,
							courtLongitude);
					if (distance <= 5) {
				...
				for (FuelStation fuelStation : fuelStationsList) {
					...
					Double distance = locationsService.getDistanceByRadian(latitide, longitude, stationLatitide,
							stationLongitude);
					if (distance <= 5) {
				...
				for (Motels motel : motelsList) {
					...
					Double distance = locationsService.getDistanceByRadian(latitide, longitude, motelLatitide,
							motelLongitude);
					if (distance <= 5) {
