...
@Injectable()
export class DestinationInspectionServices {
    ...
    sendEmail(loadNumber) {
        return this.http.post(endponitConfig.SOLAR_LOAD_ENDPOINT + 'reports/expensesReport/Expenses_Report', { "RP_loadnum": loadNumber }, { headers: this.headers })
            .map(res => { return [] });
    }

    validateCartonSkidLevel(loadNumber, cartonQr, InspectionType, skidId) {
        return this.http.get(endponitConfig.SOLAR_CARTONS + 'getCartonDetailsByCartonIdInSkidLevel/' + cartonQr + '/' + loadNumber + '/' + InspectionType + '/' + skidId, { headers: this.headers })
    ...
    skidDropsByLoadNumber(loadNumber: any) {
        // tslint:disable-next-line:max-line-length
        return this.http.get(endponitConfig.LOAD_API + 'skidDropsByLoadNumber/' + loadNumber, { headers: this.headers })
            .map(res => { return res.json().data }).catch(this.handleError);
    }
