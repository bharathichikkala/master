...
export class ExpensesDetailsComponent {
    ...
    constructor(private router: Router, private expenses_service: ExpensesServices, private route: ActivatedRoute, private _location: Location) { }
    ...
    loadDetails(loadNumber: any) {
        this.expenses_service.getLoadDetails(loadNumber).subscribe(
            data => {
                if (data) {
                  console.log(data);
    ...
    loadAccept(loadNumber: any) {
        ...
        if (!checkLoadAccepted) {
            this.expenses_service.setLoadAccept(loadNumber, 4).subscribe(
                data => {
                    this.loadDetailsData = data;
    ...
    updateDriverCoordinates() {
        ...
        this.expenses_service.sendDriverCoordinates(this.loadDetailsData.originLocNbr.latitude,
    ...
    getTimeandDistanceValues(pLat, pLng, dLat, dLng) {
        this.expenses_service.getTimeandDistance(pLat, pLng, dLat, dLng).subscribe(response => {
