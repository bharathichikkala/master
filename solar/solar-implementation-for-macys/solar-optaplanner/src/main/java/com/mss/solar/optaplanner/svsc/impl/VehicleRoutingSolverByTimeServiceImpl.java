...
import com.mss.solar.optaplanner.svsc.VehicleRoutingSolverByTimeService;


@RestController
@Validated
public class VehicleRoutingSolverByTimeServiceImpl implements VehicleRoutingSolverByTimeService {

	private static final Logger log = Logger.getLogger(VehicleRoutingSolverByTimeServiceImpl.class);
	...
	@Override
	public synchronized VehicleRoutingSolution retrieveOrCreateSolutionByTime(String loadNumber) {
		log.info("retrive or create a solution");
		VehicleRoutingSolution solution = sessionSolutionMap.get(loadNumber);
		if (solution == null) {
			File inputFile=new File(vrpFile+"/"+loadNumber+"-road-time-n-k1.vrp");
		if (!inputFile.exists()) {
	...
	@Override
	public synchronized boolean solveByTime(String loadNumber) {
        ...
        solver.addEventListener(event -> {
            ...
            synchronized (VehicleRoutingSolverByTimeServiceImpl.this) {
        ...
        final VehicleRoutingSolution solution = retrieveOrCreateSolutionByTime(loadNumber);
	...
	@Override
	public synchronized boolean terminateEarlyByTime(String loadNumber) {
