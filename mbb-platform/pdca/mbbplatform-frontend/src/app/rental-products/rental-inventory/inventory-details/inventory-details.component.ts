...
import { RentalService } from '../../../rental-products/rental-products.service';
...
export class InventoryDetailsComponent {
    ...
    listDetails = [{ id: 1, list_name: 'Available' }, { id: 5, list_name: 'Pending QC' }, { id: 6, list_name: 'Unavailable' },
    { id: 8, list_name: 'Demo' }, { id: 9, list_name: 'Rental' }]
    @ViewChild('historyModal') public historyModal: ModalDirective;
    @ViewChild('showHistoryModal') public showHistoryModal: ModalDirective;
    @ViewChild('lgModal') public lgModal: ModalDirective;
    headers = new HttpHeaders().set('Authorization', JSON.parse(sessionStorage.getItem("Autherization")))
    constructor(private readonly rentalService: RentalService, private readonly http: HttpClient, public router: Router, private readonly route: ActivatedRoute) {
    }
    ...
    inventoryDetails = {
        ...
        ajax: (data, callback, settings) => {
            this.http.get(`${endponitConfig.RENTALS_API}getRentalItemsBasedOnSkuCodeAndDates/${this.QueryDataStartDate}/${this.QueryDataEndDate}/${this.sku}/${this.id}/${this.locationId}`,
        ...
        columns: [
            ...
            },
            {
                ...
                "render": (data, type, row, meta) => {
                    if (row['history'] != null) {
                        return '<a class="eye" title="Show"> <i class="fa fa-eye"></i></a> / <a class="add" title="View History">Add</a>'
                    }
                    return '<a class="add" title="View History">Add</a>'
                }
            }
        ],
        buttons: [
    ...
    barcode: any;
    add(data) {
        this.historyForm.get('history').setValue('')
        this.barcode = data.qrcode;
        this.historyModal.show();
    }
    historyDetails: any;
    getHistory(data) {
        this.barcode = data.qrcode;
        this.rentalService.getHistory(this.barcode).subscribe((data: any) => {
            if (data.error == null) {
                this.historyDetails = data.data;
                this.showHistoryModal.show();
            } else {
    ...
    rentalSuccess; rentalFail; submitted = false; btnDisable = false; history;
    addHistory() {
        this.btnDisable = true;
        if (this.historyForm.valid) {
            this.rentalService.addHistory(this.history, this.barcode).subscribe((data: any) => {
                if (data.error == null) {
                    this.rentalSuccess = 'History Added Successfully';
                    setTimeout(() => {
    ...
    getExportFileName() {
        if (this.locationId === 0) {
            ...
            return 'Rental_Inventory_Report' + this.formatedDate()
        }
    }
    formatedDate() {
        const date = new Date(),
