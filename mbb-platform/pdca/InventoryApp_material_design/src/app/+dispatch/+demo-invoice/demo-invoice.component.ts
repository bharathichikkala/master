...
export class DemoDispatchComponent implements OnInit {
    ...
    constructor(
        ...
        this.invoiceForm = this.fb.group({
            ...
            demoGivenBy: [null, Validators.required],
            // image: [null],
    ...
    ngOnInit() {
        ...
        this.payId = '';
        this.channelId = ''
        this.appservice.getAllDemoPaymentModes().subscribe((data: any) => {
            if (data.error == null) {
    ...
    receiptImg: any = ''; img: any; selectedImage: any;
    takePicture(field) {
        ...
        if (navigator != undefined) {
            ...
            navigator.camera.getPicture(
                (base64string) => {
                    ...
                    //     + base64string);            
                    this.postObj.proofs = base64string;
                },
                (error) => {
                    this.receiptImg = '';
    ...
    demoInvoiceSelection: boolean;
    filteredDemoInvoicesList;
    demoInvoiceId;
    dispatchedDemoInvoices;
    getAllDemoProducts() {
        this.appservice.getAllDemoProductsToBeDispatched().subscribe(data => {
            if (data.error == null) {
    ...
    navigatetoInvoice() {
        if (this.invoiceForm.valid) {
            ...
            if ((this.channelId == 1 && this.payId != 8) || (this.channelId == 7 && this.payId != 8)) {
                this.appservice.getImageId(this.postObj).subscribe((data: any) => {
                    if (data.error == null) {
    ...
    freeDemo: boolean = false;
    getDemoProduct(e) {
        this.appservice.getDemoProductById(e).subscribe(data => {

            if (data.error != null) {
                this.errorMessage = data.error.message;
                setTimeout(() => {
                    this.errorMessage = null;
                    this.invoiceForm.get('invoice').setValue('');
                }, 3000)
            }
            else {
                // alert("jhhh")
                ...
                if (data.data.freeDemo) {
                    ...
                    // this.payId = 11;
                    this.freeDemo = data.data.freeDemo;
                    this.invoiceForm.get('payId').setValue(11);
                    this.paymentModeList = [{ "id": 11, "types": "FREE_DEMO" }];
                }
                else {
                    // alert(data.data.freeDemo)
                    this.freeDemo = false;
